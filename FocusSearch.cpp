...
void FocusSearch::initializeM0() {
    ...
    if ( g_n > h_n ) { return; }
    M0_list.assign(g_n, {});
    std::vector<std::vector<int>> G_sorted_neighbor_degrees(g_n);
    for (int i = 0; i < g_n; i++) {
        for (int n : G.neighbours(i+1)) {
            G_sorted_neighbor_degrees[i].push_back(G.degree(n));//we only need the degree of the neighbour
        }
        std::sort(G_sorted_neighbor_degrees[i].rbegin(), G_sorted_neighbor_degrees[i].rend());
    }
    std::vector<std::vector<int>> H_sorted_neighbor_degrees(h_n);
    for (int j = 0; j < h_n; j++) {
        for (int n : H.neighbours(j+1)) {
            H_sorted_neighbor_degrees[j].push_back(H.degree(n));
        }
        std::sort(H_sorted_neighbor_degrees[j].rbegin(), H_sorted_neighbor_degrees[j].rend());
    }
    for ( int i = 0; i < g_n; i++ ) {
        for ( int j = 0; j < h_n; j++ ) {
            ...
            if ( G.degree(i+1) <= H.degree(j+1) ) {
                ...
                for (size_t k = 0; k < G_sorted_neighbor_degrees[i].size(); k++) {
                    if (G_sorted_neighbor_degrees[i][k] > H_sorted_neighbor_degrees[j][k]) {
                        contender = false;
                        break;
                    }
                }
